...
;; The search state is bound here, not assigned, so that concurrent calls
;; each search in tables of their own.
(defun bicomponents (v gr)
  (let ((bicomp ())
        (*dfs-bicomp-depth* 0)
        (*dfs-bicomp-num* (make-hash-table))
        (*dfs-bicomp-low-pt* (make-hash-table))
        (*dfs-bicomp-edges* (make-hash-table))
        (*bicomponents* ()))
    (dolist (v (graph-vertices gr))
...
(defmfun $strong_components (gr)
  ...
  (if (= 0 (digraph-order gr))
      `((mlist simp))
      (let ((res)
	    (*scon-low* (make-hash-table))
	    (*scon-dfn* (make-hash-table))
	    (*scon-comp* ())
	    (*scon-st* ())
	    (*scon-vrt* (digraph-vertices gr))
	    (*scon-depth* 0))
...
(defmfun $max_clique (gr)
  ...
  (let ((*maximum-clique* ()) (v) (coloring) (h ($copy_graph gr)))
